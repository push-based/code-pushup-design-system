...
import {
  WorkspaceValidationPluginOptions,
  audits,
  groups,
  pluginSlug as slug,
  workspaceValidationPlugin,
} from './workspace-validation.plugin';
  ...
  it(
    ...
    async () => {
      ...
      await expect(executePlugin(pluginConfig)).resolves.toMatchObject({
        slug,
        description:
          'A plugin to measure a custom nx workspace setup alignment.',
        title: 'Workspace validation',
        icon: 'typescript',
...
describe('recommendedRefs', () => {
  it.each(generateCategoriesRefs())(
