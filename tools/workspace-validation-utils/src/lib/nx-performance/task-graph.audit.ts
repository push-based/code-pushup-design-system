...
export async function taskGraphAudits(options?: TaskGraphAuditOptions): Promise<AuditOutputs> {
...
export async function taskGraphTiming(tasks: string[]): Promise<{ duration: number; task: string }[]> {
    const results: { duration: number; task: string }[] = [];
    const isWindows = process.platform === 'win32';
    for (const task of tasks) {
        const start = performance.now();
        await promisify(execFile)('npx', ['nx', 'run-many', '-t', task, '--graph', join('tmp', 'nx-performance', 'task-graph', task, 'graph.json')], {
            shell: isWindows,
            env: {
                ...process.env,
